...
function DashboardApp() {
    ...
    const handleTagAssetAsCash = async (asset, isCash) => {
        console.log(`Tentando marcar '${asset.ticker || asset.name}' como caixa=${isCash}`);
        const identifier = asset.ticker || asset.name;
        if (!identifier) {
            alert('Não foi possível identificar o ativo.');
            return;
        }
        try {
            const payload = { assetIdentifier: identifier, isCash };
            ...
            console.error("Erro ao marcar ativo como caixa:", error);
            alert(error.message);
        }
    };
